      ...
      <Section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 max-w-6xl w-full">
          ...
          <div className="lg:col-span-2">
            <GlassCard className="h-full flex flex-col" data-testid="contact-info-card">
              ...
              <div className="space-y-5 flex-1">
                {contactInfo.map((item) => (
                  <div key={item.label} className="flex items-center gap-4" data-testid={`contact-info-${item.label.toLowerCase()}`}>
                    <div className="w-10 h-10 flex items-center justify-center bg-[var(--gold)] text-[var(--void)] flex-shrink-0">
              ...
              <div className="mt-8 pt-6 border-t border-[var(--border-subtle)]">
                <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-3">Follow Us</p>
                <div className="flex gap-3 flex-wrap">
                  {[
                    ...
                    <a
                      ...
                      className="px-4 py-2 text-xs font-medium text-[var(--text-secondary)] bg-[var(--surface-2)] border border-[var(--border-subtle)] hover:border-[var(--border-gold-active)] hover:text-[var(--gold)] transition-all"
                    >
                      {s.label}
                    </a>
                  ))}
            ...
            <GlassCard data-testid="contact-form-card">
              {isSubmitted ? (
                <div className="text-center py-12" data-testid="contact-success-message">
                  <div className="w-16 h-16 mx-auto mb-6 flex items-center justify-center bg-[rgba(212,175,55,0.1)] border border-[var(--border-gold)]">
                  ...
                  <Button onClick={() => setIsSubmitted(false)} variant="outline" data-testid="contact-send-another">
                ...
                <form onSubmit={handleSubmit} className="space-y-6" data-testid="contact-form">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">First Name</label>
                      <input type="text" required placeholder="Arjun" data-testid="contact-first-name" className="w-full input-field" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">Last Name</label>
                      <input type="text" required placeholder="Sharma" data-testid="contact-last-name" className="w-full input-field" />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">Email Address</label>
                    <input type="email" required placeholder="arjun@company.com" data-testid="contact-email" className="w-full input-field" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">Project Type</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">Budget Range (Optional)</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2 block">Message</label>
                    <textarea required rows={5} placeholder="Tell us about your project..." data-testid="contact-message" className="w-full input-field resize-none" />
                  </div>
                  <button
                    type="submit"
